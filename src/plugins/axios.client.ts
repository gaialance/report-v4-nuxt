...
export default defineNuxtPlugin(() => {
  ...
  const fetchResponse = async <T>(config: FetchConfig): Promise<AxiosResponse<T>> => {
    ...
    switch (fetchType) {
      case FetchType.Get:
        return await apiService.get<T>(url, requestConfig)

      case FetchType.Post:
        return await apiService.post<T>(url, data, requestConfig)

      case FetchType.Put:
        return await apiService.put<T>(url, data, requestConfig)

      case FetchType.Delete:
        return await apiService.delete<T>(url, requestConfig)
    }
  }
