<template>
    <VueDatePicker ref="datepickerValue" month-name-format="short" v-model="dates" :month-change-on-scroll="false" range hide-offset-dates
    close-on-scroll teleport-center :is-24="false">
        <template #left-sidebar>
            <div class="left-sidebar mt-2" @click="updateDates(preset.range)" v-for="preset of presetRanges" :key="preset.label">
                <p class="orange-text mb-2">{{preset.label}}</p>
                <v-divider color="black"/>
            </div>
        </template>
        
        <template #action-buttons>
            <v-btn @click="selectDate">Select</v-btn>
        </template>
